...
<h3><i class="fas fa-users"></i> <b><?php echo $settings['site_name'] ?> Admins & Staff</b></h3></div>
<?php
$postData = $statusMsg = '<div class="info-msg">
<p><i class="fa fa-info-circle"></i> Create a new Admin or Staff Member</p></div>';  
...
'" . mysqli_real_escape_string($conn, $_POST['admin_role']) . "',
...
<tr>
<td><h3>Account Role</h3></td>
</tr>
<tr>
<td>
<label for="admin_role"></label>
<select name="admin_role" id="admin_role">
<option value="Admin">Admin</option>
<option value="Staff">Staff</option>
</select> 
    Is Admin or Staff (Staff has access to Cats, Docs and Updates only)</td>
</tr>
<tr>
...
<?php
$result = mysqli_query($conn, "SELECT * FROM admins WHERE admin_role = 'Admin' ORDER BY admin_username ASC") 
...
echo "<tr td align='left'> <th><p>Admin Name</p></th> <th><p>Admin Email</p></th> <th><p>Admin Role</p></th> <th><p>Status</p></th> <th><p>Edit</p></th> <th><p>Delete</p></th></tr>";
while($row = mysqli_fetch_array( $result )) {
echo "<tr>";
echo '<td align="left" valign="top"><p>' . $row['admin_username'] . '</p></td>';
echo '<td align="left" valign="top"><p>' . $row['admin_email'] . '</p></td>';
echo '<td align="left" valign="top"><p>' . $row['admin_role'] . '</p></td>';
...
<hr />
<div class="heading_box">
<h3><i class="fas fa-cog"></i> <b>Manage <?php echo $settings['site_name'] ?> Staff Accounts</b></h3></div>

<?php
$result = mysqli_query($conn, "SELECT * FROM admins WHERE admin_role = 'Staff' ORDER BY admin_username ASC") 
or die(mysqli_error($conn));  
$numrows = mysqli_num_rows($result);
if($numrows == 0){ 
echo "<i class='fa fa-info-circle'></i> You currently have no Staff Accounts in the database."; 
} 
else
{
echo "<table border='0' cellpadding='3' table width='100%'>";
echo "<tr td align='left'> <th><p>Staff Name</p></th> <th><p>Staff Email</p></th> <th><p>Staff Role</p></th> <th><p>Status</p></th> <th><p>Edit</p></th> <th><p>Delete</p></th></tr>";
while($row = mysqli_fetch_array( $result )) {
echo "<tr>";
echo '<td align="left" valign="top"><p>' . $row['admin_username'] . '</p></td>';
echo '<td align="left" valign="top"><p>' . $row['admin_email'] . '</p></td>';
